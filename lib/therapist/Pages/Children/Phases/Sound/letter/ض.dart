...
class LetterDaD extends StatefulWidget {
  late String childId;
  LetterDaD({required this.childId});

  @override
  State<LetterDaD> createState() => _LetterDaDState();
}

class _LetterDaDState extends State<LetterDaD> {
  ...
  void _loadValues() async {
    try {
      ...
      if (snapshot.exists) {
        setState(() {
          final checkboxesMap = snapshot.get('ض'); //هنا يتم تغير الحرف
