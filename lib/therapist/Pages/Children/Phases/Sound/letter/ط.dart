...
class LetterTA extends StatefulWidget {
  late String childId;
  LetterTA({required this.childId});

  @override
  State<LetterTA> createState() => _LetterTAState();
}

class _LetterTAState extends State<LetterTA> {
  ...
  void _loadValues() async {
    try {
      ...
      if (snapshot.exists) {
        setState(() {
          final checkboxesMap = snapshot.get('ط'); //هنا يتم تغير الحرف
