...
class LetterThad extends StatefulWidget {
  late String childId;
  LetterThad({required this.childId});

  @override
  State<LetterThad> createState() => _LetterThadState();
}

class _LetterThadState extends State<LetterThad> {
  ...
  void _loadValues() async {
    try {
      ...
      if (snapshot.exists) {
        setState(() {
          final checkboxesMap = snapshot.get('ظ'); //هنا يتم تغير الحرف
