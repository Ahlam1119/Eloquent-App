...
class LetterA extends StatefulWidget {
  late String childId;
  LetterA({required this.childId});

  @override
  State<LetterA> createState() => _LetterAState();
}

class _LetterAState extends State<LetterA> {
  ...
  void _loadValues() async {
    try {
      ...
      if (snapshot.exists) {
        setState(() {
          final checkboxesMap = snapshot.get('أ'); //هنا يتم تغير الحرف
