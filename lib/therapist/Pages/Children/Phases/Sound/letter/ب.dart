...
class LetterB extends StatefulWidget {
  late String childId;
  LetterB({required this.childId});

  @override
  State<LetterB> createState() => _LetterBState();
}

class _LetterBState extends State<LetterB> {
  ...
  void _loadValues() async {
    try {
      ...
      if (snapshot.exists) {
        setState(() {
          final checkboxesMap = snapshot.get('ب'); //هنا يتم تغير الحرف
