...
class LetterGain extends StatefulWidget {
  late String childId;
  LetterGain({required this.childId});

  @override
  State<LetterGain> createState() => _LetterGainState();
}

class _LetterGainState extends State<LetterGain> {
  ...
  void _loadValues() async {
    try {
      ...
      if (snapshot.exists) {
        setState(() {
          final checkboxesMap = snapshot.get('غ'); //هنا يتم تغير الحرف
