...
class LetterHA extends StatefulWidget {
  late String childId;
  LetterHA({required this.childId});

  @override
  State<LetterHA> createState() => _LetterHAState();
}

class _LetterHAState extends State<LetterHA> {
  ...
  void _loadValues() async {
    try {
      ...
      if (snapshot.exists) {
        setState(() {
          final checkboxesMap = snapshot.get('ه'); //هنا يتم تغير الحرف
