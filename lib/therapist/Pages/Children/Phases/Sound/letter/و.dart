...
class LetterWow extends StatefulWidget {
  late String childId;
  LetterWow({required this.childId});

  @override
  State<LetterWow> createState() => _LetterWowState();
}

class _LetterWowState extends State<LetterWow> {
  ...
  void _loadValues() async {
    try {
      ...
      if (snapshot.exists) {
        setState(() {
          final checkboxesMap = snapshot.get('و'); //هنا يتم تغير الحرف
